...
/*   Updated: 2021/04/01 12:55:20 by hherin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	public:
		...
		void	cliConnect()
		{
			...
			while (true){
				...
				// _readfd = master;
				int selectval;
				if ((selectval = select(FD_SETSIZE, &_readfd, NULL, NULL, &tv)) < 0){
				...
				if (selectval == 0)
				{
					...
					for (int i = 1; i < MAX_CONNECTIONS; i++){
						if (all_connections[i] > 0 && FD_ISSET(all_connections[i], &_readfd)){
							...
							n = write(all_connections[i],"I got your message\n",19);
